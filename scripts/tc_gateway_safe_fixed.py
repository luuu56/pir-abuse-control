...
Fixed version:
  - Dynamic source-level blocking is thresholded by default.
  - Optional shadow mode records verifier BLOCK signals without installing drops.
  - HTTP control API exposes /metrics, /blocks, and /clear.
  - eBPF filter only applies to TCP traffic whose destination port is verifier port 8002.
  - FIX: /metrics no longer deadlocks. The previous version used a normal Lock and
    metrics() called blocks(), which tried to acquire the same Lock again.
"""
...
class GatewayState:
    def __init__(self, bpf: BPF, args: argparse.Namespace):
        ...
        # Use RLock because metrics() may need helper methods that inspect shared state.
        # A normal Lock can deadlock if the same thread tries to re-enter it.
    ...
    def _blocks_locked(self) -> list[dict]:
    ...
    def blocks(self) -> list[dict]:
        with self.lock:
            return self._blocks_locked()

    def metrics(self) -> dict:
        with self.lock:
            active_blocks = self._blocks_locked()
            return {
                "ok": True,
